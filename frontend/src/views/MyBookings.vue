...
import { ArrowRight, Star, X, Gift } from 'lucide-vue-next'
...
onMounted(async () => {
  ...
  else if (bookings.value.some(b => b.status === 'cancelled')) activeTab.value = 'cancelled'
})

const pendingList   = computed(() => bookings.value.filter(b => b.status === 'pending').sort((a,b) => new Date(a.starts_at).getTime() - new Date(b.starts_at).getTime()))
const confirmedList = computed(() => bookings.value.filter(b => b.status === 'confirmed').sort((a,b) => new Date(a.starts_at).getTime() - new Date(b.starts_at).getTime()))
const completedList = computed(() => bookings.value.filter(b => b.status === 'completed').sort((a,b) => new Date(b.starts_at).getTime() - new Date(a.starts_at).getTime()))
const cancelledList = computed(() => bookings.value.filter(b => ['cancelled','no_show'].includes(b.status)).sort((a,b) => new Date(b.starts_at).getTime() - new Date(a.starts_at).getTime()))

const tabs = computed(() => [
  { key: 'pending'   as const, label: 'На рассмотрении', count: pendingList.value.length },
  { key: 'confirmed' as const, label: 'Подтверждённые',  count: confirmedList.value.length },
  { key: 'completed' as const, label: 'Завершённые',     count: completedList.value.length },
  { key: 'cancelled' as const, label: 'Отменённые',      count: cancelledList.value.length },
])

const currentList = computed(() => {
  if (activeTab.value === 'pending')   return pendingList.value
  if (activeTab.value === 'confirmed') return confirmedList.value
    ...
    <div class="my-container">
      ...
      <template v-else>
        ...
        <div v-if="autoReschedules.length > 0" class="banners-wrap">
          <div v-for="ar in autoReschedules" :key="ar.id" class="info-banner info-banner--blue">
            ...
            <div class="info-banner__body">
              ...
              <p class="info-banner__detail">
                <b>{{ ar.service_name }}</b> · <b>{{ ar.new_master_name }}</b> · {{ formatDateTime(ar.new_starts_at) }}
              </p>
        ...
        <div v-if="waitlist.length > 0" class="waitlist-wrap">
          ...
          <div v-for="w in waitlist" :key="w.id" class="waitlist-row">
            ...
            <button class="btn btn-ghost btn-sm" @click="leaveWaitlist(w.id)">{{ t('myBookings.waitlist_leave') }}</button>
          </div>
        </div>
        ...
        <div v-if="bookings.length === 0" class="empty-state">
          ...
          <button class="btn btn-primary" style="margin-top:20px" @click="router.push('/book')">
            {{ t('myBookings.book_btn') }} <ArrowRight :size="16" />
          </button>
        </div>
        ...
        <template v-else>
          ...
          <div v-if="currentList.length === 0" class="empty-tab">
            <p>Нет записей в этом разделе</p>
          </div>

          <div v-else class="bookings-list">
            <div v-for="b in currentList" :key="b.id" class="booking-item">
              ...
              <div class="booking-item__body">
                ...
                <div class="booking-item__actions-mobile">
                  ...
                  <button
                    ...
                    @click="openReview(b)"
                  ><Star :size="14" /> {{ t('myBookings.review_btn') }}</button>
                </div>
              </div>
      ...
      <div v-if="showReschedule" class="modal-overlay" @click.self="showReschedule = false">
        <div class="modal-box card">
          ...
          <div class="reschedule-discount">
            <Gift :size="20" stroke="var(--success)" fill="none" />
            <div>
              <p style="font-size:13px;font-weight:600;color:var(--success)">{{ t('myBookings.reschedule_discount_title') }}</p>
              <p style="font-size:11px;color:var(--text-3);margin-top:1px">{{ t('myBookings.reschedule_discount_sub') }}</p>
            </div>
          </div>
          <div style="display:flex;flex-direction:column;gap:16px;margin-top:16px">
            <div class="field">
              <label class="field-label">{{ t('myBookings.reschedule_master') }}</label>
              <select v-model="newMasterID" class="field-input field-select" @change="rescheduleSlots = []; newTime = ''">
            ...
            <div class="field">
              ...
              <input v-model="newDate" type="date" :min="today" class="field-input" @change="newTime = ''; loadRescheduleSlots()" />
            </div>
            <div v-if="newDate">
              <label class="field-label" style="display:block;margin-bottom:10px">{{ t('master.reschedule_time') }}</label>
              <div v-if="rescheduleSlots.length === 0" style="font-size:13px;color:var(--text-3)">Нет свободных слотов</div>
              <div v-else class="slots-grid">
                <button
                  v-for="slot in rescheduleSlots" :key="slot.starts_at"
                  class="slot-chip" :class="{ 'slot-chip--active': newTime === formatTime(slot.starts_at) }"
                  @click="newTime = formatTime(slot.starts_at); newSlotStartsAt = slot.starts_at"
                >{{ formatTime(slot.starts_at) }}</button>
              </div>
            </div>
            <div v-if="rescheduleError" style="color:var(--danger);font-size:13px;padding:8px 12px;background:var(--danger-bg);border-radius:var(--radius-sm)">{{ rescheduleError }}</div>
            <div class="modal-footer-btns">
              <button class="btn btn-outline" @click="showReschedule = false">{{ t('common.cancel') }}</button>
              <button class="btn btn-primary" :disabled="!newDate || !newTime || rescheduling" @click="confirmReschedule">
      ...
      <div v-if="showReview" class="modal-overlay" @click.self="showReview = false">
        <div class="modal-box card">
          <div class="modal-header">
            ...
            <button class="btn btn-ghost btn-icon" @click="showReview = false"><X :size="16" /></button>
          </div>
          <p style="font-size:13px;color:var(--text-2);margin-bottom:24px">
            {{ reviewTarget?.service_name }} · {{ reviewTarget?.master_name }}
          </p>
          <div style="margin-bottom:24px">
            <label class="field-label" style="display:block;margin-bottom:12px">{{ t('myBookings.review_rating') }}</label>
            <div class="stars-row">
              <button
                v-for="n in 5" :key="n" class="star-btn"
                :style="{ color: n <= (hoverRating || reviewRating) ? '#E8442A' : 'var(--border-strong)', transform: n <= (hoverRating || reviewRating) ? 'scale(1.1)' : 'scale(1)' }"
                @click="reviewRating = n" @mouseenter="hoverRating = n" @mouseleave="hoverRating = 0"
              ><Star :size="16" /></button>
            </div>
          </div>
          <div class="field" style="margin-bottom:24px">
            <label class="field-label">{{ t('myBookings.review_comment') }}</label>
            <textarea v-model="reviewComment" class="field-input" rows="3" :placeholder="t('myBookings.review_placeholder')" style="resize:none" />
          </div>
          <div class="modal-footer-btns">
            ...
            <button class="btn btn-primary" :disabled="reviewSending" @click="submitReview">
              ...
              {{ t('myBookings.review_submit') }} <ArrowRight :size="16" />
            </button>
          </div>
        </div>
      </div>
...
.tab-btn {
  padding: 10px 16px; font-size: 13px; font-weight: 600; letter-spacing: 0.03em;
...
.tab-count {
  ...
  background: var(--accent); color: #fff; margin-left: 6px; padding: 0 4px;
}
.tab-btn:not(.tab-btn--active) .tab-count { background: var(--border); color: var(--text-2); }

.empty-tab { text-align: center; padding: 48px 0; color: var(--text-3); font-size: 14px; }

.btn-review { background: var(--accent); color: #fff; border: none; font-weight: 600; }
.btn-review:hover { background: #c93820; color: #fff; }

.my-page { min-height: calc(100vh - 56px); background: var(--bg); }
.my-container { max-width: 800px; margin: 0 auto; padding: 48px 40px 80px; }

.page-head { display: flex; justify-content: space-between; align-items: flex-end; padding-bottom: 32px; }
.page-state { text-align: center; padding: 80px; color: var(--text-3); font-size: 14px; }

.banners-wrap { margin-bottom: 32px; }
.info-banner { display: flex; align-items: center; gap: 14px; padding: 16px 20px; border-radius: var(--radius-sm); border: 1px solid; margin-bottom: 12px; }
...
.waitlist-row { display: flex; justify-content: space-between; align-items: center; padding: 14px 0; border-top: 1px solid var(--border); }
...
.booking-item__time  { font-family: var(--font-serif); font-size: 28px; font-weight: 700; color: var(--text); line-height: 1; }
.booking-item__day   { font-size: 11px; font-weight: 500; letter-spacing: 0.04em; color: var(--text-3); text-transform: capitalize; }
...
.reschedule-discount { display: flex; align-items: center; gap: 12px; padding: 12px 14px; background: var(--success-bg); border: 1px solid rgba(45,106,63,0.2); border-radius: var(--radius-sm); }

.empty-state { text-align: center; padding: 80px 0; display: flex; flex-direction: column; align-items: center; }
.empty-state__num   { font-family: var(--font-serif); font-size: 100px; font-weight: 900; color: var(--text); opacity: 0.04; line-height: 1; margin-bottom: -20px; }
...
.modal-footer-btns { display: flex; gap: 10px; justify-content: flex-end; padding-top: 12px; border-top: 1px solid var(--border); }
.stars-row { display: flex; gap: 6px; }
.star-btn  { font-size: 32px; background: none; border: none; cursor: pointer; transition: transform 0.1s; line-height: 1; padding: 0; min-width: 44px; min-height: 44px; display: flex; align-items: center; justify-content: center; }
...
@media (max-width: 600px) {
  ...
  .booking-item { grid-template-columns: 80px 1fr; gap: 14px; padding: 16px 0; }
  .booking-item__right { display: none; }
  .booking-item__actions-mobile {
    display: flex; align-items: center; gap: 8px; flex-wrap: wrap;
    margin-top: 10px; padding-top: 10px; border-top: 1px solid var(--border);
  }
  .booking-item__price-mobile { font-family: var(--font-serif); font-size: 16px; font-weight: 700; color: var(--text); margin-left: auto; }
  ...
  .modal-box { position: fixed !important; bottom: 0 !important; left: 0 !important; right: 0 !important; top: auto !important; max-width: 100% !important; border-radius: var(--radius-sm) var(--radius-sm) 0 0 !important; max-height: 90vh; overflow-y: auto; }
